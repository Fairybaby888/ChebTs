...
# ==================== 路径配置 ====================
DATA_DIR = "/kaggle/input/datasets/baby9966/liver-data/"
...
# ============================================================================
# 1. ChebNetII 优化版 Chebyshev 卷积 
# ============================================================================
class ChebConvII_Optimized(nn.Module):
    """
    ChebNetII: 优化版 Chebyshev 卷积，默认K=5
    """
...
# ============================================================================
# 2. 融合模型：ChebNetII + TransformerConv + PE/SE
# ============================================================================
class ChebNetII_Transformer_PESE(nn.Module):
    """
    融合模型：
    - 局部分支：ChebNetII（K=5）
    - 全局分支：TransformerConv（图注意力）
    - 支持 PE/SE 注入
    - 注意力池化用于图读出
    """
    ...
    def forward(self, x, edge_index, pe, se, edge_weight=None, lambda_max=None):
        ...
        # 6. 基因重要性（用于注意力池化）
        importance = self.gene_importance(h).squeeze(-1)  # [N]
        
        # 7. 注意力池化（替代 mean pooling）
        # 方法1：基于基因重要性的加权平均
        attention_weights = F.softmax(importance, dim=0)  # [N]
        graph_repr = (h * attention_weights.unsqueeze(1)).sum(dim=0, keepdim=True)  # [1, D]
...
# ============================================================================
# 3. 离线计算函数：Laplacian PE 和 Random Walk SE
# ============================================================================
...
def load_sample_labels():
    """加载 GSE54236 样本标签 (81 Tumor, 80 NonTumor)"""
    df = pd.read_csv(DATA_DIR + "GSE54236_sample_groups.csv")
    ...
    if 'Dataset' in df.columns:
        df = df[df['Dataset'] == 'GSE54236'].copy()
    ...
    # 创建标签: 根据 Group 列判断（修复版）
    def get_label(group_value):
        group_str = str(group_value).lower().strip()
        
        # 非肿瘤样本: 匹配 'non tumor' 或 'non-tumor' 或 'nt'
        if 'non tumor' in group_str or 'non-tumor' in group_str:
            return 0
        # 非肿瘤样本: 以 'nt' 结尾或等于 'nt'
        if group_str.endswith('nt') or group_str == 'nt':
            return 0
        # 肿瘤样本: 包含 'tumor'
        if 'tumor' in group_str:
            return 1
        # 默认
...
def load_expression_data():
    """加载表达矩阵"""
    expr = pd.read_csv(DATA_DIR + "GSE54236_expression.csv", index_col=0)
...
def main(use_cv=True, n_splits=5):
    print("=" * 70)
    print("ChebNetII + TransformerConv + PE/SE 模型 - 肝癌驱动基因筛选")
    print("=" * 70)
    ...
    if use_cv:
        ...
        for fold_idx, (train_val_idx, test_idx) in enumerate(skf.split(sample_ids, labels)):
            ...
            # 6:2:2 划分：从 train_val_idx 中分出 75% 训练，25% 验证
            # 由于 train_val_idx 占总样本的 (n_splits-1)/n_splits
            # 需要调整使得最终比例为 60% 训练, 20% 验证, 20% 测试
            # 设总样本为 N，测试集占 1/n_splits
            # 训练集目标 0.6N，验证集目标 0.2N
            # 所以从 train_val_idx 中取 train_size = 0.6N / ((n_splits-1)/n_splits * N) = 0.6/((n_splits-1)/n_splits)
            # 对于 n_splits=5: train_size = 0.6/(0.8) = 0.75
            # 对于 n_splits=3: train_size = 0.6/(0.667) ≈ 0.9
            if n_splits == 5:
                train_size = 0.75  # 最终 60% 训练, 20% 验证
            elif n_splits == 4:
                train_size = 0.8   # 最终 60% 训练, 20% 验证
            elif n_splits == 3:
                train_size = 0.9   # 最终 60% 训练, 20% 验证
            else:
                train_size = 0.75
    ...
    plot_gene_importance(top_genes, top_scores, avg_scores, nodes, f"{output_dir}/gene_importance.png", top_n=20)
...
if __name__ == "__main__":
    # use_cv=True 启用5折交叉验证，n_splits可改为3或4
