...
DATA_DIR_1 = "/kaggle/input/datasets/baby9966/dataset/"
...
def load_sample_labels():
    gse31905 = pd.read_csv(DATA_DIR_1 + "GSE31905_sample_groups.csv")
    gse41657 = pd.read_csv(DATA_DIR_1 + "GSE41657_sample_groups.csv")
    all_labels = pd.concat([gse31905, gse41657], ignore_index=True)
    task_labels = all_labels[all_labels['Group'].isin(['Adenoma', 'Cancer'])].copy()
    task_labels['label'] = (task_labels['Group'] == 'Cancer').astype(int)
    print(f"GSE31905: {len(gse31905)}, GSE41657: {len(gse41657)}")
    print(f"总样本: {len(task_labels)} (Adenoma: {(task_labels['label']==0).sum()}, Cancer: {(task_labels['label']==1).sum()})")
    return task_labels

def load_expression_data():
    expr = pd.read_csv(DATA_DIR_1 + "merged_combat_corrected.csv", index_col=0)
    print(f"表达矩阵: {expr.shape[0]} 基因 × {expr.shape[1]} 样本")
    return expr

def load_ppi_network():
    edges = pd.read_csv(DATA_DIR_1 + "ppi_network_edges_threshold400.csv")
    nodes = pd.read_csv(DATA_DIR_1 + "ppi_network_nodes_threshold400.csv")
    print(f"PPI: {len(nodes)} 节点, {len(edges)} 边")
    return edges, nodes
...
def prepare_data(sample_ids, expression_df, edges, nodes, label_dict, pe_k=8, se_walk=20):
    ...
    lambda_max = compute_lambda_max(edge_index, n_nodes)
    ...
    for sample_id in sample_ids:
        if sample_id in expression_df.columns:
            ...
            expr_sample = pd.Series(np.random.randn(n_nodes), index=genes_in_network)
    ...
    data_list = []
    for i, sample_id in enumerate(tqdm(sample_ids, desc='Building graph data', leave=False)):
        ...
        data.y = torch.tensor([label_dict[sample_id]], dtype=torch.long)
        data_list.append(data)
    
    return data_list, edge_index, edge_weight
...
def main(use_cv=True, n_splits=5):
    ...
    output_dir = "E:/数据集/差异表达分析/构建PPI/results_chebnetii_transformer_pese"
    ...
    all_data, edge_index, edge_weight = prepare_data(
